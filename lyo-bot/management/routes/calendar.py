...
async def api_operators(request: Request, date: str = Query(default=None)):
    ...
    # Only show "Non assegnato" column when there are actually unassigned appointments
    # on the requested date — avoids cluttering the view with an empty column.
                ...
                cur2.execute(
                    """SELECT 1 FROM appointments
                       WHERE business_id = %s AND appointment_date = %s
                         AND operator_id IS NULL AND status = 'confirmed'
                       LIMIT 1""",
                    (user["business_id"], date[:10]),
                )
                has_unassigned = cur2.fetchone() is not None
        except Exception:
            has_unassigned = True  # safe default: show column if check fails
    else:
        has_unassigned = True  # no date filter → show it always
